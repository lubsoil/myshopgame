if(!obj_game_run.game_paused){
	...
	if(worker_state == 0){
		...
		var trash_product = tryFindProductTrash();
		...
		if(free_workercashregister != noone && worker_ai_cashregister){
			...
			interaction_attempts = 0;
			person_speed = 0.65;
		}else if(trash_product != noone && worker_ai_cleaning){
			worker_state = 6;
			setObjectPathfinding(trash_product);
			target_object.trash_worker = self;
			interaction_time = 120 - skill_cleaning*20;
			interaction_attempts = 0;
			person_speed = 0.65;
	...
	}else if(worker_state == 6 && worker_ai_cleaning){
		...
		if(isTargetValid()){
			...
			setObjectPathfinding(tryFindTrash());
			if(target_object != noone){
				target_object.trash_worker = self;
			}else{
				if(interaction_attempts > 100){
			...
			interaction_time = 120 - skill_cleaning*10;
		}
