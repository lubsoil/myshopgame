...
shop_popularity = 0;
shop_popularity += (instance_number(obj_cashregister) * 0.5);
shop_popularity += (instance_number(obj_shelf) * ceil(ds_list_size(available_products)/2.5)) * (instance_number(obj_atmmachine) > 0 ? 2.5 : 1)
...
shop_popularity *= time_multiplier;

guests_minimum = 1 + floor(0.34*shop_popularity);
guests_maximum = 3 + floor(0.69*shop_popularity);